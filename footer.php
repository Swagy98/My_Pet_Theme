<footer class="bg-secondary">
  <div class="section">
    <div class="container">
      <div class="row">
        <!-- Widget Area 1 -->
        <div class="col-md-3 col-sm-6 mb-4 mb-md-0">

        <a class="navbar-brand" href="<?php echo esc_url(home_url('/')); ?>">
        <?php 
            if (has_custom_logo()) {
                 the_custom_logo();
            } else {
                echo '<img class="img-fluid" src="' . esc_url(get_template_directory_uri() . '/images/logo.png') . '" alt="' . get_bloginfo('name') . '">';
            }
        ?>
    </a>
 
        
        </div>

        <!-- Widget Area 2 -->
        <div class="col-md-3 col-sm-6 mb-4 mb-md-0">

        <?php if (is_active_sidebar('footer-1')) : ?>
              <?php dynamic_sidebar('footer-1'); ?>
          <?php else : ?>
              <a href="<?php echo esc_url(home_url('/')); ?>"><img src="<?php echo get_template_directory_uri(); ?>/images/logo.png" alt="<?php bloginfo('name'); ?>" class="img-fluid"></a>
          <?php endif; ?>

       
        </div>

        <!-- Widget Area 3 -->
        <div class="col-md-3 col-sm-6 mb-4 mb-md-0">

        <?php if (is_active_sidebar('footer-2')) : ?>
              <?php dynamic_sidebar('footer-2'); ?>
          <?php endif; ?>


         
        </div>

        <!-- Social Links or Static Content -->
        <div class="col-md-3 col-sm-6 mb-4 mb-md-0">
        <?php if (is_active_sidebar('footer-3')) : ?>
              <?php dynamic_sidebar('footer-3'); ?>
          <?php endif; ?>
        </div>
      </div>
    </div>
  </div>
</footer>

  <div class="text-center pb-3">
    <p class="mb-0">Copyright ©<script>var CurrentYear = new Date().getFullYear()
    document.write(CurrentYear)</script> a theme by  <a href="https://themefisher.com/">themefisher.com</a></p>
  </div>
</footer>

<!-- jQuery -->
<script src="<?php echo get_stylesheet_directory_uri(  ).'/assets/js/jquery.min.js';?>"></script>
<!-- Bootstrap JS -->
<script src="<?php echo get_stylesheet_directory_uri() . '/assets/js/bootstrap.min.js' ;?>"></script>
<!-- slick slider -->
<script src="<?php echo get_stylesheet_directory_uri() . '/assets/slick/slick.min.js';?>"></script>
<!-- masonry -->
<script src="<?php echo get_stylesheet_directory_uri(  ).'/assets/js/masonry.js'?>"></script>
<!-- instafeed -->
<!-- <script src="wp-content/themes/pet_theme/plugins/instafeed/instafeed.min.js"></script> -->
<!-- smooth scroll -->
<script src="<?php echo get_stylesheet_directory_uri(  ).'/assets/js/smooth-scroll.js';?>"></script>
<!-- headroom-->
<script src="<?php echo get_stylesheet_directory_uri(  ).'/assets/js/headroom.js';?>"></script>
<!--reading time -->
<!-- <script src="wp-content/themes/pet_theme/plugins/reading-time/readingTime.min.js"></script> --> -->


<!-- Main Script -->
<script src="<?php echo get_stylesheet_directory_uri(  ).'/assets/js/script.js';?>"></script>

</body>
</html>





















<!-- <footer class="bg-secondary">
  <div class="section">
    <div class="container">
      <div class="row">
        <div class="col-md-3 col-sm-6 mb-4 mb-md-0">
          <a href="index.html"><img src="images/logo.png" alt="persa" class="img-fluid"></a>
        </div>
        <div class="col-md-3 col-sm-6 mb-4 mb-md-0">
          <h6>Address</h6>
          <ul class="list-unstyled">
            <li class="font-secondary text-dark">Sydney</li>
            <li class="font-secondary text-dark">6 rip carl Avenue CA 90733</li>
          </ul>
        </div>
        <div class="col-md-3 col-sm-6 mb-4 mb-md-0">
          <h6>Contact Info</h6>
          <ul class="list-unstyled">
            <li class="font-secondary text-dark">Tel: +90 000 333 22</li>
            <li class="font-secondary text-dark">Mail: exmaple@ymail.com</li>
          </ul>
        </div>
        <div class="col-md-3 col-sm-6 mb-4 mb-md-0">
          <h6>Follow</h6>
          <ul class="list-inline d-inline-block">
            <li class="list-inline-item"><a href="#" class="text-dark"><i class="ti-facebook"></i></a></li>
            <li class="list-inline-item"><a href="#" class="text-dark"><i class="ti-twitter-alt"></i></a></li>
            <li class="list-inline-item"><a href="#" class="text-dark"><i class="ti-linkedin"></i></a></li>
            <li class="list-inline-item"><a href="#" class="text-dark"><i class="ti-github"></i></a></li>
          </ul>
        </div>
      </div>
    </div>
  </div>
  <div class="text-center pb-3">
    <p class="mb-0">Copyright ©<script>var CurrentYear = new Date().getFullYear()
    document.write(CurrentYear)</script> a theme by  <a href="https://themefisher.com/">themefisher.com</a></p>
  </div>
</footer> -->
